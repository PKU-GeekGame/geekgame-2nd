...
下载文件，复制，对照Windings表修复部分未复制到的内容，栅栏密码。

## 小北问答 · 极速版

用python的pwntools进行交互

可能的问题

1. 质数：使用sympy的isprime函数判断。然后每一个范围都有8个质数，需要反复刷
2. 电子游戏概论：前往github上第一届留档的src/pygame/game/server/libtreasure.py里，找到GOAL_OF_LEVEL，复制下来写进程序
3. bilibili：复制网上现成代码
4. PKURunner：打开手机搜索pkurunner，找到包。
5. Mac地址：不会
6. Firefox：搜索可得为65
7. gStore：访问官网，找到最古早的文献，把标题丢进搜索引擎
8. 世界一流大学：访问，然后用F12看Host

只要会7题就能拿满分，反复刷即可。

## 编原译理习题课

第一个问题搜Compiler Bomb。开头第一个就是用数组使程序爆炸。
...
后两个问题我在GCC的buglist里找到

第二个（这个是在搜第三题的时候意外发现的，忘了来自哪里，但肯定是bugtracker），原理应该是在数组里反复调用已删除的构造函数。
```C++
#define V V
#define Y typename
#define S struct
namespace std {
template<Y>S allocator;
template<>basic_istream<char>&operator>>(basic_istream<char>&__is,basic_string<char>&__str);
template<Y _Tp,Y _Alloc>S V{return*static_cast<_Tp_alloc_type*>(&this->_M_impl);V(const allocator_type &__a)~V(){}};
template<Y _Tp,Y _Alloc=std::allocator<_Tp>>S vector:protected V<_Tp,_Alloc>{}};S foo{foo::foo(){}}typedef S{std::vector <foo> foos;}b;typedef S{b B[200];}e;typedef S w{e E[200][10]};static const w W={};
```

第三个（[来自这里](https://gcc.gnu.org/bugzilla/show_bug.cgi?id=104068)）
```C++
template<typename T>
struct r{
    using i=unsigned;
...
struct c {};
template<typename T, typename T::i Extent>
struct e
{
    using t = T;
    using i = typename t::i;
    template<i O>
    constexpr e<T, O> s(c<i, O>) const noexcept
{
return {};
}
};
...
using i = typename t::i;
template<i Extent>
constexpr e<T, Extent>s(c<i, Extent>)const noexcept{return {};}
};

template struct view<r<int>>;
```
...
第二题的部分提供了一个javascript，程序中使用ScriptEngine运行它。把这个javascript复制出来，一些变量代换后可以看出该函数的作用是取未变量替换前自身部分位置的字符。提取得flag。
...
list=[0,15,16,17,30,105,16,31,16,67,3,33,5,60,4,106,6,41,0,1,67,3,16,4,6,33,232]
print(''.join(map(lambda x:s[x],list)))
```

## 智慧检测器

尝试了几下，发现有些情况下会卡墙。查看代码，可以发现423行`CurPos=NewPos`使用了一个浅拷贝，这使得如果第一步走对，第二步撞墙，撞墙前NewPos改变，CurPos也跟着变，直接育碧。找到bug后思路就明确了。

第一题随便往一个可行方向走然后UP一下（例如输入NU）。

第二题还是利用pwntools交互。此外为使得成功率提升，需要知道迷宫的生成方式。读代码可知入口在最底层一侧，出口在最高层另一侧。因此要以走到对侧的方向为最优先。接下来通不通的过就是概率问题。

## 企鹅文档

本题分为两个部分。

第一部分利用腾讯提供的api直接读取。这里抄了AnalyZL的代码，得到https://geekgame.pku.edu.cn/service/template/prob_kAiQcWHobsBzRJEs_next。访问进入第二部分。

第二部分使用Chrome打开har。仔细筛选后找到最后一次GET sheet的位置。找到返回值里initialAttributedText的位置，自己新建一个表对照后可知这是一个247行11列的表，并且腾讯文档单元格的编码是从左到右从上到下的编码（例如，本题中A1对应0，A3对应2，B2对应11*(2-1)+(2-1)=12），将有内容的单元格用程序画出就可以发现这是flag的字符画。读出即可。

## 私有笔记

第一题翻阅CVE，可以找到CVE-2021-45038，参考漏洞追踪器中T297574提供的构造方法，访问`index.php?action=rollback&from={{:flag}}`即可显示。

第二题给出提示后做出。查到CVE-2020-29007。参考漏洞追踪器T257062提供的构造，尝试几次后可以发现flag2在临时文件夹向上两格的部分。然后传入如下代码即可
...
第一题访问`/admin/query/`（注意最后的斜杠），把HTML里表单的链接也加上斜杠，然后选择PKU_GeekGame查询可得。

## 这也能卷

第一题给出找到premium.js，可以发现其中最外层有flag0。在浏览器控制台里输入flag0即可读取第一题flag。并作为激活码。

## 381654729

搜索可知这个数前n位可以整除n。参考代码可以猜想本题要求的flag要满足16进制下的关系。将原数转十六进制后为`666c616774e86912c24ad70b66ace22d54647a6b6127713f`，因此从e开始到末尾是目标数的长度，为38。写个程序dfs一下即可。
...
if __name__=="__main__":
    ...
    print(num)
    for n in num:
        i=int(n+"42",16)^2511413510804728068173995351177700473173974053702622212415
        print(i.to_bytes(24,"big"))
```

## 乱码还原

由于题目代码中使用了`decode("shift_jis",errors="ignore")`，编码时会跳过部分无法解析的字节。

首先把所有字字节的可能组合构造一个字典，接下来对应查找即可得flag1的结果。

第二题看提示后做出。AES分块使用16个字节一块。因此可以在修复过程中每获取解码后的16个字节就丢进AES解码一下看看能不能解出ASCII范围内的结果，这样筛选可以避免指数爆炸。
...
with open("flag2.enc","r",encoding="utf-8") as f:
    text=f.read()
    bs=text.encode("shift-jis")

print("end reading file")

with open("f2test.enc","wb") as f:
    f.write(bs)
...
def insertDctList(d,lst):
    for c in lst:
        b=c.encode("utf-8")
        insertDct(d,b,c)
        insertDct(d,b[1:],c)
        insertDct(d,b[:2],c)
        insertDct(d,b[0:1]+b[2:3],c)
        insertDct(d,b[0:1],c)
        insertDct(d,b[1:2],c)
        insertDct(d,b[2:3],c)
...
def createDct():
    d={}
    insertDctList(d,["："])
    insertDctList(d,TUDOU)
    insertDctList(d,BYTEMARK)
    return d
...
while True:
    ...
    #print(i,j,bs[i:j])
    if j-i>4:
        print("Error: Unknwon key",bs[i:j-1])
        for k,v in sorted(dct.items()):
            print(f'{k}\t{v}')
        exit(0)
    if bs[i:j] in dct and (j>=l or bs[i:j+1] not in dct):
        cs=dct[bs[i:j]]
        se=set(cs)
        if len(se & set(BYTEMARK))>1:
            cs=''.join(se&set(TUDOU))+BYTEMARK[0]
        r.append(cs)
        print(f"{i}/{l}\t{bs[i:j]}")
        i=j
    j=j+1
print("end fix lose bytes")
print("Some Info:")
print(f"With multiple choices count:{len(tuple(filter(lambda x:len(x)>1,r)))}")

def dumpAllAns(choices):
    ans=['']
    l=len(choices)
    for j,cs in enumerate(choices):
        tmp=[]
        if len(cs)==1:
            ...
            for a in ans:
                for c in cs:
                    if a and a[-1] in BYTEMARK and c in BYTEMARK:
                        continue
                    s=a+c
                    if s[-1] in BYTEMARK:
                        tmp.append(s)
...
#with open("f1diff.enc","w",encoding="utf-8") as f:
#    f.write(s.encode("utf-8").decode("shift_jis",errors="ignore"))
...
解码后根据密文的特征反复调用对应的base解码，解码顺序为base32，ascii85，base85,base16，ascii85，base64，ascii85，base64，ascii85，base64。

## 奇怪的加密

说实话，第一题我本来是一点思路也没有的。但运气很不错，在“1863”这个数字附近出现了两个完全一致的序列`Twhpyxq`，并且搜索可以知道这是Kasiski，可以得到循环周期为22。借助一些猜想可得该句话为`Fredrich Kasiski in 1863 came up with the Kasiski Examination`，搜索可知该段话来自[这个PDF](https://www.lancaster.ac.uk/media/lancaster-university/content-assets/documents/cyber-foundry/Cryptography.pdf)。比较后可以发现该段原文与原话对应。利用这一点统计各个位置上字母的对应关系，即可得到各个位置的加密表。找到各个位置的密文表中前一步和后一步键值相同的点即可得到对应的key。代码如下。注意要把这段原文中两个`è`改成`e`。

```python
lowle="abcdefghijklmnopqrstuvwxyz"
caple=lowle.upper()
...
cipher=preprocess("Put your Cipher Text Here")
plain=preprocess("Put your Plain Text Here")

l=22
rings=[{} for _ in range(l)]
rever=[{} for _ in range(l)]
for i in range(len(plain)):
    o=i%l
    if plain[i] in rings[o]:
        if rings[o][plain[i]]==cipher[i]:
            continue
        else:
            print("Error")
            exit(0)
    else:
        rings[o][plain[i]]=cipher[i]
    if cipher[i] in rever[o]:
        if rever[o][cipher[i]]==plain[i]:
            continue
        else:
            print("Error")
            exit(0)
    else:
        rever[o][cipher[i]]=plain[i]
...
while not testIdent(rings[0]):
    rings=rings[1:]+[rings[0]]
    rever=rever[1:]+[rever[0]]

keymap={i:None for i in lowle}
for i in range(l):
    before=rings[i]
    after=rings[(i+1)%15]
    for c in set(before)&set(after):
        ...
        if keymap[b]:
            if keymap[b]!=a:
                print("Error")#虽然不知道为啥有bug，但反正能用。
                print(sorted(keymap.items()))
                print(''.join(map(lambda x:x[0],sorted(keymap.items(),key=lambda x:x[1]))))
                exit(0)
        else:
            keymap[b]=a
    print(keymap)
...
按词义和词首字母转即可得到flag。（参考：[北约音标字母](https://baike.baidu.com/item/%E5%8C%97%E7%BA%A6%E9%9F%B3%E6%A0%87%E5%AD%97%E6%AF%8D)）

第二题可以看出是16进制。因此每个位置只可能是`abcdef`中的一种，首先先确定周期，通过统计`a-f`字符的位置可以判断周期为26。然后每个位置上一步中的字母可以映射到下一步即`6x6=36`种，求各步映射的交集即可得结果。
...
cipher=preprocess("Use your cipher text here")
...
s=0
while s<=nochange[-1]:
    if s not in nochange:
        print("Error")
        exit(0)
    s+=l
print("len: ",l)

canBe=[set() for _ in range(l)]
for i,c in enumerate(cipher):
    canBe[i%l].add(c)

wait={i:set(lowle) for i in lowle}
...
ident={i:wait[i].pop() for i in wait}
rever={ident[i]:i for i in ident}

print(''.join(map(lambda x:x[1],sorted(rever.items()))).upper())
```

解码后把每行丢进去一搜可以发现是md5的加密，打一个字典反查就行。注意其中出现了长度为3个字母的md5。
...
go语言的随机数种子只有低31位有用，因此我们只需要遍历所有可能的种子即可得到结果。第一题直接暴力破解，跑了个10分钟跑出答案。说真的，2^31不大。
...
#define RNG_MASK 0x7fffffffffffffff

#include<stdio.h>

const long long COOKED[RNG_LEN] = {
...
unsigned long long getui64() {
    long long res;
    --TAP;
    if (TAP < 0)TAP += RNG_LEN;
    --FEED;
    if (FEED < 0)FEED += RNG_LEN;
    res = VEC[FEED] + VEC[TAP];
    VEC[FEED] = res;
    return (unsigned long long)res;
}

long long geti63() {
    return (long long)(getui64() & RNG_MASK);
}

int geti31() {
    return (int)(geti63() >> 32);
}
...
int geti31n(int n) {
    ...
    tmp = ((unsigned int)1 << 31) % (unsigned int)n;
    max = (int)(INT32_MAX - tmp);
    while ((v = geti31()) > max);
    return v % n;
}
...
int testSeed(int seed) {
    int i;
    if((seed&0x1fffff)==0)printf("%d\n", seed>>21);
    initRnd(seed);
    for (i = 0; i < BOARD_SIZE; ++i) {
        if (BOARD[i] != (getin(257) % 2))return 0;
    }
    return 1;
}
...
#define STEP 134217728
//1~16

int main() {
    int seed = N*STEP-1; 
    //编译时另外一个脚本会在本代码前面加一个"#define N num"，num为0~15
    //编译完后同时运行这16个程序，最差一个半小时出答案，我暴的两次都是10分钟。
    //不想研究并行，就直接把并行的问题交给操作系统
    int end=seed-1+STEP;
    int i, j;
    for (i = 0; i < BOARD_SIZE; ++i)scanf("%d", BOARD + i);
    while (1) {
        if (testSeed(seed))printf("Seed: %d\a\n", seed);
        if (seed == end)break;
        ++seed;
    }
    while (getchar() != 'e');
...
第二题的随机数使用uint64，读源码可知，产生的uint64会重新填回数组中。每一个扫雷盘面产生4个uint64，而数组长度为607，因此循环152次后我们就得到了一个完全确定的数组。操作较多，这里改用python交互。开机之前记得先点一下level2。
...
VEC=[0 for _ in range(RNG_LEN)]
HAS=[False for _ in range(RNG_LEN)]
...
domain="https://prob14-put_your_url_here.geekgame.pku.edu.cn/"
...
for _ in range(TURN):
    ...
    while True:
        x=pos>>4
        y=pos&0xF
        r=requests.post(domain+"click",data={'x':str(x),'y':str(y)})
        d=r.json()
        if "boom" in d:
            us=d['boom']
            u1=dumpull(us[0:4])
...
Level3把Level1的程序改一改就好了，忽略那些与securerand搞起来的位置。反正种子只有2^31，结果有2^158，跑出来一个种子基本上就肯定是那个种子。扫完之后手动扫雷即可。

## 方程组

第一题满秩直接解。

第二题读一下后发现距离满秩差4个数，遍历36^4可解。